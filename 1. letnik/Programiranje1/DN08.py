...
def dolzina_ovir(ovire):
    return sum([0 + x1 - x0 + 1 for x0, x1, _ in ovire])
...
def indeksi(s, subs):
    return [i for i in range(len(s)) if s.startswith(subs, i)]

def pretvori_vrstico(vrstica):
    return list(zip([x+1 for x in indeksi("."+ vrstica + ".", ".#")], [y for y in indeksi("." + vrstica + ".", "#.")]))   
