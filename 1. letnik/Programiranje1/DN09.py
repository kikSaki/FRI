...
def vrednost_povezave(povezava, zemljevid):
    return sum(točke[key] for key in zemljevid[povezava])  

def najboljsa_povezava(zemljevid):
    return max(povezava for povezava in zemljevid if vrednost_povezave(povezava, zemljevid) == (max(vrednost_povezave(povezava, zemljevid) for povezava in zemljevid)))

def vrednost_poti(pot, zemljevid):
    return sum(vrednost_povezave((pot[i],pot[i+1]), zemljevid) for i in (range(len(pot)-1)) if (pot[i],pot[i+1]) in zemljevid)

def najbolj_uporabna(pot, zemljevid):
    b=[]
    pogosto = collections.defaultdict(int)
    if len(pot)-1 == 0:
        return None

    for i in (range(len(pot)-1)):
        a=(pot[i],pot[i+1])
        for j in zemljevid[a]:
            b.append(j)
...
def mozna_pot(pot, zemljevid, vescine):
    ...
    for i in (range(len(pot)-1)):
        ...
        if a not in zemljevid:
            return False
        else:
            for j in zemljevid[a]:
                if j in vescine: 
                    vescine.remove(j)                  
                elif j not in vescine:
                    return False
    return True
...
def koncna_tocka(pot, zemljevid, vescine):
    ...
    for i in (range(len(pot)-1)):
        ...
        if a not in zemljevid:  
            ...
            for j in zemljevid[a]:
                for skill, kolk in nove:                   
                    if j in skill:
                        if kolk < 1:
                            return pot[i]                       
                        nove[nove.index((skill, kolk))] = (skill, kolk-1)
    if len(pot)==2:
        return pot[-1]
    return pot[i]
...
def do_nagrade(pot, zemljevid, meja):
    ...
    for i in (range(len(pot)-1)):
        ...
        if a not in zemljevid:
            ...
            sum += vrednost_povezave(a,zemljevid)
...
def naslednje_tocke(tocke, zemljevid, vescine):
    return set(neki if neki != k[0]  else k[1] for neki in tocke for k in zemljevid if ((neki,k[1]) in zemljevid) if (zemljevid[(neki, k[1])] - vescine == set()) )

def dosegljive(tocka, zemljevid, vescine):

    if naslednje_tocke(tocka, zemljevid, vescine) == tocka:
        return tocka

    else:
        for vsaka in tocka:
            tocka = set(tocka).union(naslednje_tocke(vsaka, zemljevid, vescine))
        return dosegljive(tocka, zemljevid, vescine)
