...
def mozen_skok(x, y, ovira):
    if ovira[0]-y > 2 or ovira[0] - y < -2:
        return False
    elif ovira[0] == y and (x+2 >= ovira[1] and x+2 <= ovira[2]):
        return True
    elif (ovira[0]-y == 2 or ovira[0]-y == -2)  and x < ovira[2]:
        if x+1 >= ovira[1]:
            return True
        if x+2 >= ovira[1]:
            return False
    elif (ovira[0]-y == 1 or ovira[0]-y == -1)  and x < ovira[2]:
        if x+1 >= ovira[1] or x+2 >= ovira[1]:
            return True
    else:
       return False
...
def mozne_ovire(x,y):
    a = []
    for i in ovire:
        if mozen_skok(x,y,i) is True:
            a.append(i)
    return a

def obstaja_pot(ovira):
    
    if ovira[2] == 16 or ovira[2] == 17:
        return True
    
    elif mozne_ovire(ovira[2], ovira[0]) == []:
        return False
    
    else:
        for i in mozne_ovire(ovira[2],ovira[0]):
            if obstaja_pot(i) == True:
                return True
        return False
